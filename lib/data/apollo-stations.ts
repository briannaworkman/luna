...
export interface ApolloStation {
  mission: string;
  station: string;
  lat: number;
  lon: number;
...
export function findNearestStation(lat: number, lon: number): (ApolloStation & { distanceKm: number }) | null {
  let nearestStation: ApolloStation | null = null;
  ...
  return nearestStation ? { ...nearestStation, distanceKm: nearestDist } : null;
}
