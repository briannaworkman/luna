...
export async function runMissionHistoryAgent(input: {
  ...
  const { system, user } = buildMissionHistoryPrompt({ dataContext })
  ...
  try {
    ...
    for await (const ev of stream) {
      ...
      if (parsed.text.length > 0) {
        emit({ type: 'agent-chunk', agent: 'mission-history', text: parsed.text })
      }
      for (const c of parsed.citations) {
        emit({ type: 'agent-citation', agent: 'mission-history', source: c.source, id: c.id })
      }
      // Mission history does not use confidence scoring — suppress any [CONFIDENCE:] tags
      // the model may emit despite the system prompt instruction.
    }

    // Flush any trailing carry (prose tail with no closing tag)
    if (carry.length > 0) {
      emit({ type: 'agent-chunk', agent: 'mission-history', text: carry })
    }
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
    emit({ type: 'agent-error', agent: 'mission-history', message })
  }
}
